...
description: Guidelines for code structure, testing requirements, task completion protocol, and AI behavior when contributing to the project.
---
...
1. **Setup Development Environment (Always First)**
   IMPORTANT: Use a worktree.
   ```bash
   skill: git-worktree
...
2. **Search for Past Solutions**
   ...
   - Use `grep_search` with those keywords to find files
   - Read the content of those files

3. **Implementation & Testing**
   Follow the `<Contributing Guidelines>`, `<code-structure>`, and `<testing-requirements>` detailed below. Plan features vertically.

4. **Task Completion**
   Validate and track progress according to the `<task-completion>` protocols.

5. **Document Learnings**
   After successfully completing the task, document any new learnings, solutions, or patterns that might apply in the future using:
   ```bash
